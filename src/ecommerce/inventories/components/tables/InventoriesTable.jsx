...
import DeleteInventoryModal from '../modals/DeleteInventoriesModal';
import { DeleteOneInventory } from '../../../remote/del/DeleteOneInventory';
    ...
    return (
        <Box>
                ...
                <Stack direction="row" sx={{ m: 1 }}>
                  <Box>
                    ...
                    <Tooltip title="Editar">
                      <IconButton
                           onClick={() => {
                               if (productSel) setUpdateModalOpen(true);   // abre el modal
                             }}>
                        <EditIcon />
                      </IconButton>
                    </Tooltip>
          ...
          <UpdateInventoryModal
              open={updateModalOpen}                       // controla visibilidad
              onClose={() => setUpdateModalOpen(false)}    // cierra modal
              inventoryData={productSel}                   // datos de la fila seleccionada
              onUpdated={() => {                           // callback tras PUT exitoso
                fetchData();                               // refresca la tabla
                setUpdateModalOpen(false);                 // cierra modal
              }}
          />
          <DeleteInventoryModal
            open={deleteDialogOpen}
            onClose={() => setDeleteDialogOpen(false)}
            message={`¿Seguro que deseas eliminar el almacén ${toDelete?.IdAlmacenOK}?`}
            onConfirm={async () => {
              await DeleteOneInventory(toDelete.IdAlmacenOK);   // cliente Axios
              setDeleteDialogOpen(false);
              fetchData();                                      // recarga tabla
            }}
          />
        </Box>
      );
